...
int main()
{
  ...
  Math::Vector3(-1.3f,  1.0f, -1.5f)
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightShader.Use();
		Math::Vector3 lightPos(cos(glfwGetTime())*5, 0, sin(glfwGetTime())*2);
		...
		for (size_t i = 0; i < 10; i++)
		{
			...
			model.Rotate(Math::Vector3(1, 1, 1),glfwGetTime()*36);
			...
			//环境光
			shader.SetVector3("ambient", Math::Vector3(0.1, 0.1, 0.1));
			
			//方向光
			shader.SetInt("directionalLightNum", 1);
			shader.SetVector3("directionalLights[0].direction", Math::Vector3(-1, -1, -1));
			shader.SetVector3("directionalLights[0].diffuse", Math::Vector3(0.5f, 0.5f, 0.5f));
			shader.SetVector3("directionalLights[0].specular", Math::Vector3(1.0f, 1.0f, 1.0f));
			//点光源
			shader.SetInt("pointLightNum",1);
			shader.SetVector3("pointLights[0].position", lightPos);
			shader.SetVector3("pointLights[0].diffuse", Math::Vector3(0.5f, 0.5f, 0.5f));
			shader.SetVector3("pointLights[0].specular", Math::Vector3(1.0f, 1.0f, 1.0f));
			shader.SetFloat("pointLights[0].constant", 1.0f);
			shader.SetFloat("pointLights[0].linear", 0.09f);
			shader.SetFloat("pointLights[0].quadratic", 0.032f);
			//聚光
			shader.SetInt("flashLightNum", 1);
			shader.SetVector3("flashLights[0].position", camera.GetTransform().Position());
			shader.SetVector3("flashLights[0].direction", camera.GetTransform().Forward());
			shader.SetFloat("flashLights[0].innerCone", cos(Math::Radians(10)));
			shader.SetFloat("flashLights[0].outerCone", cos(Math::Radians(15)));
			shader.SetVector3("flashLights[0].diffuse", Math::Vector3(0.5f, 0.5f, 0.5f));
			shader.SetVector3("flashLights[0].specular", Math::Vector3(1.0f, 1.0f, 1.0f));
			shader.SetFloat("flashLights[0].constant", 1.0f);
			shader.SetFloat("flashLights[0].linear", 0.09f);
			shader.SetFloat("flashLights[0].quadratic", 0.032f);
			// bind diffuse map
